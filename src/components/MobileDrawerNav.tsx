      ...
      {items.map((item) => {
        ...
        return (
          ...
          >
            ...
            {isActive && AccentIcon && (
              <AccentIcon className="ml-auto h-4 w-4 text-primary/60" />
            )}
            {isActive && !AccentIcon && (
              <Sprout className="ml-auto h-4 w-4 text-primary/60" />
            )}
          </Link>
        );
      })}
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetTrigger asChild>
        <Button
          size="icon"
          className="fixed bottom-6 left-6 z-50 h-14 w-14 rounded-full shadow-lg bg-primary hover:bg-primary/90 md:hidden"
      ...
      <SheetContent side="left" className="w-80 p-0">
        <SheetHeader className="p-6 pb-4">
          <SheetTitle className="flex items-center gap-3">
            <div className="flex items-center justify-center h-10 w-10 rounded-xl gradient-delta shadow-glow">
        ...
        <ScrollArea className="h-[calc(100vh-5rem)] px-4 py-4">
          <div className="space-y-6">
            ...
            <NavSection title="Delta Command Center" items={commandCenterItems} />
